...
# Write the role password to a psql variables file instead of passing it as a
# process argument (process args are readable by other processes in the pod).
DBUSER_PWD_VARS_FILE=$(mktemp)
chmod 600 "$DBUSER_PWD_VARS_FILE"
printf "\\set dbuserpwd '%s'\n" "$DBUSER_PWD" > "$DBUSER_PWD_VARS_FILE"

PGPASSWORD="$SU_USER_PWD" psql -v ON_ERROR_STOP=1 --username="$SU_USER" --host="$DB_SERVERIP" --port="$DB_PORT" --dbname="$DEFAULT_DB_NAME" -f "$DBUSER_PWD_VARS_FILE" -f role_dbuser.sql -v dbuname="$DB_UNAME"

rm -f "$DBUSER_PWD_VARS_FILE"
