...
2. Resolves the **plugin** (`mosipid` | `mock`) via `EsignetUtil.getPluginName()` — auto-detected from the eSignet actuator, no config property. Under `mosipid` it additionally provisions Keycloak users, policies, and biometric test data up front. `isSunbirdAuthenticatorActive()` gives finer detail (a Sunbird RC-backed server still reports `mock` here).
...
- **No `pluginToExecute` config property** — matches eSignet's own `api-test` reference exactly. `getPluginName()` returns `mock`/`mosipid` purely from the actuator; Sunbird RC is a finer server-side detail on top (`isSunbirdAuthenticatorActive()`), still reported as `mock` by `getPluginName()`.
