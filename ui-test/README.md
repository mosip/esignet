...
`isSunbirdAuthenticatorActive()`) by default. For deployments with no Spring actuator (e.g.
Thunder/eSignet-go), set `esignetActuatorEnabled=false` in `config.properties` and short-circuit
detection by setting `pluginToExecute` to `mosipid`/`mock` directly; `isSunbirdAuthenticatorActive()`
still always queries the actuator regardless of `pluginToExecute`.
