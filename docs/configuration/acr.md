...
(`esignet-service/internal/clientmgmt/validate.go`). Which allow-list applies depends on the API
path, not the profile: `ValidateCreate` (`POST /client-mgmt/client` and its deprecated
per-profile aliases) always uses the broader `allowedACRAll`. `ValidateUpdate`/`ValidatePatch` also
use `allowedACRAll`, **except** `PUT /client-mgmt/oidc-client/{client_id}` (the deprecated
OIDC-profile update alias), which is restricted to the narrower `allowedACROIDCPut`.
