...
| [@mosip/secure-biometric-interface-integrator](https://github.com/mosip/mosip-sdk) | ^0.1.0 | The UI SBI (Secure Biometric Interface) plugin — a vanilla JS library that bridges oidc-ui's biometric-authentication screen to the MOSIP Secure Biometric Interface device SDK for capturing a scan. | [MPL-2.0](https://github.com/mosip/mosip-sdk/blob/master/LICENSE) |
...
| Java / [Maven](https://maven.apache.org/) | 21 | `ui-test/` — the UI automation project driving `oidc-ui` end to end — is a Java 21, Maven-built module, separate from the Go backend's own test tooling above. | [GPL-2.0+CE](https://openjdk.org/legal/gplv2+ce.html) (Java) / [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0) (Maven) |
| [Selenium](https://www.selenium.dev/) | 4.14.1 | Selenium is a browser automation framework; `ui-test/` uses `selenium-java` to drive the browser through login/OTP/consent screens. | [Apache License 2.0](https://github.com/SeleniumHQ/selenium/blob/trunk/LICENSE) |
| [Cucumber](https://cucumber.io/) | 7.21.1 | Cucumber runs Gherkin feature files as executable UI test scenarios (`cucumber-java`/`-junit`/`-testng`/`-picocontainer`). | [MIT License](https://github.com/cucumber/cucumber-jvm/blob/main/LICENSE) |
| [TestNG](https://testng.org/) | 7.7.1 | TestNG provides the test runner, annotations, and suite configuration `ui-test/` executes its Cucumber scenarios under. | [Apache License 2.0](https://github.com/testng-team/testng/blob/master/LICENSE.txt) |
| [ExtentReports](https://www.extentreports.com/) | 5.1.2 | ExtentReports (via `extentreports-cucumber7-adapter`) generates the HTML test report for each `ui-test/` run. | [Apache License 2.0](https://github.com/extent-framework/extentreports-java/blob/master/LICENSE) |
| [BrowserStack Java SDK](https://www.browserstack.com/docs/automate/selenium/java-testhub) | 1.30.9 (optional) | Runs the same `ui-test/` suite against BrowserStack's cloud device/browser grid instead of a local browser, when enabled. | — |
| Mock MDS | 1.3.0 | A mock Mobile/Biometric Device Service used by `ui-test/` to simulate biometric device capture without physical hardware. | — |
| apitest-commons (MOSIP) | 1.7.0 | Shared MOSIP test-automation utilities (config/property handling, common assertions) reused by `ui-test/`. | — |
