...
Token/OIDC transaction lifetimes:
...
| `MOSIP_ESIGNET_CAPTCHA_SITE_PROVIDER` | — | none — expanded into `<DATA_DIR>/flows/flow-esignet.yaml`, not read by Go config |
...
The following discovery-related `oauth:` keys have no env override; edit `deployment.yaml`:
...
IAM client ID exactly matches either its `azp` claim or one of its `aud` values, and it has the scope
...
| `REDIS_DB` | `redis.db` | `0` |
| `REDIS_TLS_ENABLED` | `redis.tls` | `false` (auto for `rediss://`) — plaintext otherwise; production deployments must use a `rediss://` `REDIS_URL` or set this explicitly |
...
| Variable | YAML key | Default |
|---|---|---|
| `DATABASE_URL` | `db.dsn` | `""`; `sslmode=disable` appended if not already specified — production deployments should set an explicit `sslmode` (e.g. `require`/`verify-full`) instead of relying on this default |
...
events aren't published to an external audit manager, but are still logged locally via the
application logger). Any other value fails startup.
...
| Key | Default | Source |
|---|---|---|
| `DEFAULT_LANG` | `en` | `oidc-ui/public/env-config.js` |
| `DEFAULT_WELLKNOWN` | URL-encoded JSON array of `{name, value}` discovery-document links | `oidc-ui/public/env-config.js` |
| `DEFAULT_THEME` | `""` | `oidc-ui/public/env-config.js` |
| `DEFAULT_FAVICON` | `favicon.ico` | `oidc-ui/public/env-config.js` |
| `DEFAULT_TITLE` | `eSignet` | `oidc-ui/public/env-config.js` |
| `DEFAULT_FONT_URL` | Google Fonts Montserrat URL | `oidc-ui/public/env-config.js` |
| `DEFAULT_ID_PROVIDER_NAME` | `eSignet` | `oidc-ui/public/env-config.js` |
| `POLLING_URL` | `<api-base>/actuator/health` | `oidc-ui/Dockerfile` ARG, written to `env.env` |
| `POLLING_INTERVAL` | `10000` (ms) | `oidc-ui/Dockerfile` ARG, written to `env.env` |
| `POLLING_TIMEOUT` | `5000` (ms) | `oidc-ui/Dockerfile` ARG, written to `env.env` |
| `POLLING_ENABLED` | `true` | `oidc-ui/Dockerfile` ARG, written to `env.env` |

`oidc-ui/public/env-config.js` only ships the `DEFAULT_*` checked-in defaults; editing it does not
change the `POLLING_*` values — those are Dockerfile build ARGs baked into `env.env`, which
`configure_start.sh` merges with live environment variables to generate the runtime
`env-config.js` at container start (see "Container config" below).
...
overrides at container start. `configure_start.sh` fetches these with `wget --no-check-certificate`
— TLS certificate validation is disabled for these downloads regardless of URL, so only point them
at trusted hosts.
