	...
	public void test(TestCaseDTO testCaseDTO)
		...
		if (testCaseName.contains("ESignet_")) {
			...
			if ((testCaseName.contains("_AuthorizationCode_")) || (testCaseName.contains("_AuthToken_Xsrf_"))
					|| (testCaseName.contains("_OAuthDetailsRequest_")) || (testCaseName.contains("_GenerateLinkCode_"))
					|| (testCaseName.contains("_LinkTransaction_"))
				...
				if (EsignetUtil.getIdentityPluginNameFromEsignetActuator().toLowerCase()
						...
						|| (EsignetUtil.getIdentityPluginNameFromEsignetActuator().toLowerCase()
								...
								&& testCaseName.contains("ESignet_CreateOIDCClientV2SunBirdC_all_Valid_Smoke_sid"))) {
					...
					if (testCaseName.toLowerCase().contains("_sid")) {
						JSONObject respJson = new JSONObject(response.getBody().asString());
						if (respJson.has(GlobalConstants.RESPONSE) && !respJson.isNull(GlobalConstants.RESPONSE)) {
							JSONObject respObj = respJson.getJSONObject(GlobalConstants.RESPONSE);
							if (respObj.has(idKeyName)) {
								writeAutoGeneratedId(testCaseName, idKeyName, respObj.getString(idKeyName));
							}
						}
