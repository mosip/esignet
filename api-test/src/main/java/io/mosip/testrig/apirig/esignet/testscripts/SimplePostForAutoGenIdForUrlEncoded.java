...
public class SimplePostForAutoGenIdForUrlEncoded extends EsignetUtil implements ITest {
	...
	public void test(TestCaseDTO testCaseDTO)
		...
		String jsonInput = inputJsonKeyWordHandeler(EsignetUtil.inputstringKeyWordHandeler(inputJson, testCaseName), testCaseName);
		...
		if (testCaseDTO.getTemplateFields() != null && templateFields.length > 0) {
			...
			if (testCaseName.contains("ESignet_")) {
				...
				if (testCaseName.contains("_DPoP_AuthToken_Xsrf_")) {
					response = postWithBodyAndHeaderForAutoGeneratedIdForUrlEncoded(endPoint, jsonInput,
							testCaseDTO.getTestCaseName(), idKeyName);

					if (testCaseName.toLowerCase().contains("_smoke") && response != null
							&& response.getBody().asString().contains("use_dpop_nonce") && hasDpopNonce(response)) {
						logger.info("use_dpop_nonce detected in response for test case: " + testCaseName);
						saveDpopNonce(response, testCaseName);
						jsonInput = inputJsonKeyWordHandeler(
								EsignetUtil.inputstringKeyWordHandeler(inputJson, testCaseName), testCaseName);
						response = postWithBodyAndHeaderForAutoGeneratedIdForUrlEncoded(endPoint, jsonInput,
								testCaseDTO.getTestCaseName(), idKeyName);
					}
				} else {
