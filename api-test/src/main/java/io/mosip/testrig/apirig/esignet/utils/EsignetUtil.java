...
//			BaseTestCase.setSupportedIdTypes(Arrays.asList("UIN", "VID"));
	...
	// Cache to store PKCE parameters (code_verifier, code_challenge) per session
	private static Map<String, Map<String, String>> pkceCache = new HashMap<>();
	...
	public static String inputstringKeyWordHandeler(String jsonString, String testCaseName) {
		...
		if (jsonString.contains("$DPOP_JKT$")) {
			try {
				String jwkKeyString;
		...
		if (jsonString.contains("$DPOP_PROOF$")) {
		    try {
		        String jwkKeyString;
				...
				String tokenEndpoint = getValueFromEsignetWellKnownEndPoint("token_endpoint", EsignetConfigManager.getEsignetBaseUrl());

				if (autoGeneratedIDValueCache.get(testCaseName + "_dpopNonce") != null) {
					dpopProof = createDPoPProof(dpopKey, "POST", tokenEndpoint,
							autoGeneratedIDValueCache.get(testCaseName + "_dpopNonce"), null);
				} else {
					dpopProof = createDPoPProof(dpopKey, "POST", tokenEndpoint, null, null);
				}
				jsonString = replaceKeywordValue(jsonString, "$DPOP_PROOF$", dpopProof);
			} catch (Exception e) {
				logger.error("Error while generating DPoP JKT or Proof: " + e.getMessage(), e);
			}
		}
		
		if (jsonString.contains("$DPOP_PROOF_WITH_ACCESS_TOKEN$")) {
			try {
				String jwkKeyString;
				...
				logger.info("JWK = " + jwkKeyString);

				RSAKey dpopKey = RSAKey.parse(jwkKeyString);

				String dpopProof = null;
				String htuURL = getValueFromEsignetWellKnownEndPoint("userinfo_endpoint", EsignetConfigManager.getEsignetBaseUrl());

				JSONObject request = new JSONObject(jsonString);
				String dpopAccessToken = null;
				if (request.has(EsignetConstants.DPOP_ACCESS_TOKEN)) {
					dpopAccessToken = request.get(EsignetConstants.DPOP_ACCESS_TOKEN).toString();
				}

				logger.info("autoGeneratedIDValueCache map = " + autoGeneratedIDValueCache);

				if (autoGeneratedIDValueCache.get(testCaseName + "_dpopNonce") != null) {
					dpopProof = createDPoPProof(dpopKey, "GET", htuURL,
							autoGeneratedIDValueCache.get(testCaseName + "_dpopNonce"), dpopAccessToken);
				} else {
					dpopProof = createDPoPProof(dpopKey, "GET", htuURL, null, dpopAccessToken);
				}
				jsonString = replaceKeywordValue(jsonString, "$DPOP_PROOF_WITH_ACCESS_TOKEN$", dpopProof);
			} catch (Exception e) {
				logger.error("Error while generating DPoP JKT or Proof: " + e.getMessage(), e);
			}
		}

		if (jsonString.contains("$DPOP_PROOF_PAR$")) {
			try {
				String jwkKeyString;
				...
				String parEndpoint = getValueFromEsignetWellKnownEndPoint("pushed_authorization_request_endpoint",
						EsignetConfigManager.getEsignetBaseUrl());

				String dpopProof = null;
				if (autoGeneratedIDValueCache.get(testCaseName + "_dpopNonce") != null) {
					dpopProof = createDPoPProof(dpopKey, "POST", parEndpoint,
							autoGeneratedIDValueCache.get(testCaseName + "_dpopNonce"), null);
				} else {
					dpopProof = createDPoPProof(dpopKey, "POST", parEndpoint, null, null);
				}
				jsonString = replaceKeywordValue(jsonString, "$DPOP_PROOF_PAR$", dpopProof);
			} catch (Exception e) {
				logger.error("Error while generating DPoP Proof for PAR: " + e.getMessage(), e);
			}
		}
		...
		if (jsonString.contains("$OIDC_JWK_KEY_DPoP$")) {
			String jwkKey = "";
			if (getTriggerESignetKeyGenForDPoP()) {
				jwkKey = JWKKeyUtil.generateAndCacheJWKKey(OIDC_JWK_FOR_DPoP);
				setTriggerESignetKeyGenForDPoP(false);
			} else {
				jwkKey = JWKKeyUtil.getJWKKey(OIDC_JWK_FOR_DPoP);
			}
		...
		if (jsonString.contains("$OIDC_JWK_KEY_FAPI$")) {
			...
			if (getTriggerESignetKeyGenForFAPI()) {
				jwkKey = JWKKeyUtil.generateAndCacheJWKKey(OIDC_JWK_FOR_FAPI);
				setTriggerESignetKeyGenForFAPI(false);
			} else {
		...
		if (jsonString.contains("$OIDCJWKKEY12$")) {
			String jwkKey = "";
			if (gettriggerESignetKeyGen36()) {
				jwkKey = JWKKeyUtil.generateAndCacheJWKKey(OIDCJWK12);
				settriggerESignetKeyGen36(false);
			} else {
				jwkKey = JWKKeyUtil.getJWKKey(OIDCJWK12);
			}
			jsonString = replaceKeywordValue(jsonString, "$OIDCJWKKEY12$", jwkKey);
		}		
		
		if (jsonString.contains("$OIDCJWKKEY13$")) {
			String existingKey = JWKKeyUtil.getJWKKey(OIDCJWK13);
			String jwkKey;
		...
		if (jsonString.contains("$CLIENT_ASSERTION_PAR_JWT_EXPIRED$")) {
			String oidcJWKKeyString = JWKKeyUtil.getJWKKey(OIDC_JWK_FOR_PAR);
			try {
				oidc_JWK_Key_For_PAR = RSAKey.parse(oidcJWKKeyString);
			} catch (java.text.ParseException e) {
			...
			if (clientId != null) {
				jsonString = replaceKeywordValue(jsonString, "$CLIENT_ASSERTION_PAR_JWT_EXPIRED$",
						signJWKKeyExpired(clientId, oidc_JWK_Key_For_PAR, tempUrl));
			} else {
				logger.error("Client ID not found in JSON for $CLIENT_ASSERTION_PAR_JWT_EXPIRED$.");
			}
		}

		if (jsonString.contains("$CLIENT_ASSERTION_DPoP_JWT$")) {
			String oidcJWKKeyString = JWKKeyUtil.getJWKKey(OIDC_JWK_FOR_DPoP);
			logger.info("oidcJWKKeyString =" + oidcJWKKeyString);
			try {
				oidc_JWK_Key_For_DPoP = RSAKey.parse(oidcJWKKeyString);
				logger.info("oidc_JWK_Key_For_DPoP =" + oidc_JWK_Key_For_DPoP);
			} catch (java.text.ParseException e) {
				logger.error(e.getMessage());
			}
			...
			if (clientId != null) {
				jsonString = replaceKeywordValue(jsonString, "$CLIENT_ASSERTION_DPoP_JWT$",
						signJWKKey(clientId, oidc_JWK_Key_For_DPoP, tempUrl));
			} else {
				logger.error("Client ID not found in JSON for $CLIENT_ASSERTION_DPoP_JWT$.");
			}
		}		
	...
	public static String signJWKKeyExpired(String clientId, RSAKey jwkKey, String tempUrl) {
		...
		try {
			...
			JWTClaimsSet claimsSet = new JWTClaimsSet.Builder().subject(clientId).audience(tempUrl).issuer(clientId)
					.issueTime(issueTime).expirationTime(expirationTime).jwtID(UUID.randomUUID().toString()).build();

			logger.info("Expired client assertion JWT issue and expiry time " + issueTime + " & " + expirationTime);
	...
	private static boolean getTriggerESignetKeyGenForDPoP() {
		return triggerESignetKeyGenForDPoP;
	}
	
	private static boolean getTriggerESignetKeyGenForFAPI() {
		return triggerESignetKeyGenForFAPI;
	}
	...
	private static void setTriggerESignetKeyGenForDPoP(boolean value) {
		triggerESignetKeyGenForDPoP = value;
	}
	...
	private static void settriggerESignetKeyGen36(boolean value) {
		triggerESignetKeyGen36 = value;
	}
    ...
    public static String generateJKT(RSAKey publicKey) throws Exception {
        // JWK Thumbprint as per RFC 7638
        Base64URL thumbprint = publicKey.toPublicJWK().computeThumbprint();
        return thumbprint.toString();
    }

    // Create DPoP proof JWT
	public static String createDPoPProof(RSAKey rsaKey, String httpMethod, String url, String dpopNonce, String accessToken)
			throws Exception {
		Instant now = Instant.now();

		JWTClaimsSet.Builder claimsBuilder = new JWTClaimsSet.Builder().jwtID(UUID.randomUUID().toString())
				.issueTime(Date.from(now)).notBeforeTime(Date.from(now)).expirationTime(Date.from(now.plusSeconds(60)))
				.claim("htm", httpMethod.toUpperCase()).claim("htu", url);

		if (dpopNonce != null && !dpopNonce.isEmpty()) {
			claimsBuilder.claim("nonce", dpopNonce);
		}
		...
		JWSHeader header = new JWSHeader.Builder(JWSAlgorithm.RS256).type(new JOSEObjectType("dpop+jwt"))
				.jwk(rsaKey.toPublicJWK()).build();
	...
	protected Response postWithBodyAndHeaderForAutoGeneratedIdForUrlEncoded(String url, String jsonInput,
		...
		if (request.has(EsignetConstants.DPOP_KEY) && request.has(EsignetConstants.DPOP_VALUE)) {
			String headerName = request.getString(EsignetConstants.DPOP_KEY); // "DPoP"
			String headerValue = request.getString(EsignetConstants.DPOP_VALUE); // JWT proof

			headers.put(headerName, headerValue);

			request.remove(EsignetConstants.DPOP_KEY);
			request.remove(EsignetConstants.DPOP_VALUE);
		}

		try {
			map = mapper.readValue(request.toString(), Map.class);

			logger.info(GlobalConstants.POST_REQ_URL + url);
			logger.info("Form data for request: " + map.toString());
			logger.info("Headers for request: " + headers.toString());

			response = RestClient.postRequestWithFormDataBodyWithHeaders(url, map, headers);
	...
	public void saveDpopNonce(Response response, String testCaseName) throws AdminTestException {
	    String nonce = response.getHeader("dpop-nonce");
	    if (nonce == null || nonce.isEmpty()) {
	        logger.warn("No DPoP nonce header found in response for test case: " + testCaseName);
	        return;
	    }
	    logger.info("DPoP nonce received: " + nonce);
	    autoGeneratedIDValueCache.put(testCaseName + "_dpopNonce", nonce);
	}
    
    public boolean hasDpopNonce(Response response) {
        String nonce = response.getHeader("dpop-nonce");
        return nonce != null && !nonce.isEmpty();
    }
    
	public Response getRequestWithHeaders(String url, String jsonInput, String testCaseName)
			throws SecurityXSSException {
		Response response = null;
		jsonInput = inputJsonKeyWordHandeler(jsonInput, testCaseName);
		JSONObject requestJson = new JSONObject(jsonInput);
		HashMap<String, String> headers = new HashMap<>();
		
		if (requestJson.has(EsignetConstants.DPOP_KEY) && requestJson.has(EsignetConstants.DPOP_VALUE)) {
			String headerName = requestJson.getString(EsignetConstants.DPOP_KEY);
			String headerValue = requestJson.getString(EsignetConstants.DPOP_VALUE);

			headers.put(headerName, headerValue);

			requestJson.remove(EsignetConstants.DPOP_KEY);
			requestJson.remove(EsignetConstants.DPOP_VALUE);
		}
		
		if (requestJson.has(EsignetConstants.DPOP_ACCESS_TOKEN) && requestJson.has(EsignetConstants.AUTHORIZATION_KEY)) {
			String headerName = requestJson.getString(EsignetConstants.AUTHORIZATION_KEY);
			String headerValue = requestJson.getString(EsignetConstants.DPOP_ACCESS_TOKEN); 

			headers.put(headerName, "DPoP " + headerValue);

			requestJson.remove(EsignetConstants.AUTHORIZATION_KEY);
			requestJson.remove(EsignetConstants.DPOP_ACCESS_TOKEN);
		}

		logger.info(GlobalConstants.GET_REQ_STRING + url);
		GlobalMethods.reportRequest(null, jsonInput, url);
		try {
			response = RestClient.getRequestWithHeaders(url, MediaType.APPLICATION_JSON, MediaType.APPLICATION_JSON,
					headers);
	...
	public static JSONObject decodeUserInfoJWT(String token) throws Exception {

		String[] parts = token.split("\\.");

		// Check if token is a JWE (encrypted JWT with 5 parts)
		if (parts.length == 5) {
			...
			for (String keyName : keys) {
				try {
					...
					String jwk = JWKKeyUtil.getJWKKey(keyName);
					...
					JWEObject tempJwe = JWEObject.parse(token);
					tempJwe.decrypt(new RSADecrypter(rsaKey.toRSAPrivateKey()));
			...
			if (decrypted.split("\\.").length == 3) {
				...
				SignedJWT signedJWT = SignedJWT.parse(decrypted);

				JSONObject result = new JSONObject();
				result.put("header", signedJWT.getHeader().toJSONObject());
				result.put("payload", new JSONObject(signedJWT.getPayload().toString()));
				return result;

			} else {

				JSONObject result = new JSONObject();
				result.put("payload", new JSONObject(decrypted));
				return result;
			}
		}
		...
		// JWS decoding
			...
			JSONObject result = new JSONObject();
			result.put("header", new JSONObject(new String(Base64.getUrlDecoder().decode(jwt.getHeader()))));
			result.put("payload", new JSONObject(new String(Base64.getUrlDecoder().decode(jwt.getPayload()))));
			return result;
		}
