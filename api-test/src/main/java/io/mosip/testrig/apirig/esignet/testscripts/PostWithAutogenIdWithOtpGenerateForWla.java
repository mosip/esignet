...
public class PostWithAutogenIdWithOtpGenerateForWla extends EsignetUtil implements ITest {
	...
	public void test(TestCaseDTO testCaseDTO) throws AdminTestException, SecurityXSSException {
		...
		if (testCaseName.contains(GlobalConstants.ESIGNET_)) {
			String validateReqJson = getJsonFromTemplate(validateOtpReqJson.toString(), validateOtpReqTemplate);
			validateReqJson = EsignetUtil.inputstringKeyWordHandeler(validateReqJson, testCaseName);
			
			String tempUrl = EsignetConfigManager.getEsignetBaseUrl();
			validateOtpResponse = postRequestWithCookieAuthHeaderForAutoGenId(tempUrl + validateOtpEndPoint,
					validateReqJson, COOKIENAME, "mobileauth", testCaseDTO.getTestCaseName(), idKeyName);
		}
		...
		if (testCaseName.contains(GlobalConstants.ESIGNET_)) {
			...
			response = postWithBodyAndCookieAuthHeaderAndXsrfTokenForAutoGeneratedId(
					tempUrl + testCaseDTO.getEndPoint(), reqJson, COOKIENAME, testCaseDTO.getTestCaseName(), idKeyName);
		} else {
