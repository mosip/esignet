...
func (k *privateKey) signRSA(digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	if pssOpts, ok := opts.(*rsa.PSSOptions); ok {
		return k.signRSAPSS(digest, pssOpts)
	}
...
// signRSAPSS implements RSASSA-PSS entirely in software up to the raw RSA
// private-key operation: PSS-encode the digest locally (needs only the
// public modulus size, never the private exponent), then ask the token for a
// single mechanism-agnostic CKM_RSA_X_509 raw signature over the encoded
// block. This mirrors the "software pad, raw HSM op" split Decrypt already
// uses for OAEP in this file, for the same reason: CKM_RSA_PKCS_PSS
// mechanism/hash-agility support is unreliable across PKCS#11 tokens, while
// CKM_RSA_X_509 (plain modular exponentiation) is universally supported.
func (k *privateKey) signRSAPSS(digest []byte, opts *rsa.PSSOptions) ([]byte, error) {
	...
	case rsa.PSSSaltLengthAuto:
		saltLen = modulusSize - h.Size() - 2
	case rsa.PSSSaltLengthEqualsHash:
		saltLen = h.Size()
	}

	em, err := emsaPSSEncode(digest, rsaPub.N.BitLen()-1, saltLen, h.New())
	if err != nil {
		return nil, fmt.Errorf("pkcs11: PSS encode: %w", err)
	}
	...
	// CKM_RSA_X_509 may return fewer than modulusSize bytes on a
	// leading-zero result (same implementation-dependent behavior handled
	// for Decrypt below) — left-pad to the full modulus size.
	if len(sig) < modulusSize {
		padded := make([]byte, modulusSize)
		copy(padded[modulusSize-len(sig):], sig)
		sig = padded
	}
	return sig, nil
}
