...
func (ts *PKCS11TestSuite) TestDecodeCKULong_RealTokenByteSequences() {
	cases := []struct {
		name string
		b    []byte
		want uint
...
func (ts *PKCS11TestSuite) TestDecodeCKULong_ReproducesReportedBugValues() {
	...
	got := decodeCKULong(ecBytes)
	ts.Require().NotEqual(uint(buggyECValue), got, "decodeCKULong regressed to big-endian decoding: got the reported bug value %d", got)
	ts.Assert().EqualValues(pkcs11.CKK_EC, got, "decodeCKULong(EC bytes)")

	got = decodeCKULong(edBytes)
	ts.Require().NotEqual(uint(buggyEdValue), got, "decodeCKULong regressed to big-endian decoding: got the reported bug value %d", got)
	ts.Assert().EqualValues(ckkECEdwards, got, "decodeCKULong(Ed25519 bytes)")
}
