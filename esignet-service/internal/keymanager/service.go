...
// IsKeystoreResident is the exported form of isKeystoreResident, for
// packages outside keymanager (e.g. cryptomanager) that need to classify a
// resolved key_alias row without duplicating the hierarchy's residency
// rules.
func IsKeystoreResident(appID, refID string) bool {
	return isKeystoreResident(appID, refID)
}
...
// ThumbprintForCert is the exported form of thumbprintForCert, for
// cryptomanager to compute a thumbprint from a caller-supplied certificate
// (jwtEncrypt's x5c-bypass path) and to derive the same hex format
// resolveDecryptionKey's thumbprint lookups expect.
func ThumbprintForCert(cert *x509.Certificate) string {
...
// ResolveKeyMaterial returns the certificate and private key for an
// already-known key_alias id, exported for cryptomanager's thumbprint-based
// key resolution (see cryptomanager/keyresolve.go), which reaches a
// key_alias row by a path other than ensureCurrentKey/currentAlias and so
// cannot reuse GetCertificate/GetSigningCertificate directly. Composes the
...
// KeyPairResponse — for callers (e.g. cryptomanager.Service.Encrypt) that
...
// JWTEncrypt caller-supplied-certificate path and for decoding key_store
// certificate_data rows reached via thumbprint-based resolution.
func ParseCertPEM(data string) (*x509.Certificate, error) {
	return parseCertPEM(data)
}

// EncodeCertPEM is the exported form of encodeCertPEM.
func EncodeCertPEM(der []byte) string {
	return encodeCertPEM(der)
}
