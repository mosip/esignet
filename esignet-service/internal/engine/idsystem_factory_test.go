...
func (ts *IdsystemFactoryTestSuite) TestNewIDSystemProviders() {
	...
	t.Run("mock provider", func(t *testing.T) {
		authn, observability, err := NewIDSystemProviders(&config.AppConfig{Provider: "mock"}, nil, httpClient, nil, nil)
		if err != nil {
	...
	t.Run("sunbird provider missing config errors", func(t *testing.T) {
		...
		if _, _, err := NewIDSystemProviders(&config.AppConfig{Provider: "sunbird"}, nil, httpClient, nil, nil); err == nil {
			t.Error("expected error when sunbird search URL is unconfigured")
		}
	})

	t.Run("unsupported provider", func(t *testing.T) {
		_, _, err := NewIDSystemProviders(&config.AppConfig{Provider: "bogus"}, nil, httpClient, nil, nil)
		if err == nil {
