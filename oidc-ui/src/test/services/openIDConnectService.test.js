...
describe('openIDConnectService', () => {
  ...
  test('checkPurposeObjIsNotEmpty returns true for valid object', () => {
    expect(service.checkPurposeObjIsNotEmpty({ title: 'A' })).toBe(true);
    expect(service.checkPurposeObjIsNotEmpty({ subTitle: 'B' })).toBe(true);
    expect(service.checkPurposeObjIsNotEmpty({ type: 'C' })).toBe(true);
  });
